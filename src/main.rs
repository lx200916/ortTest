...
fn main()-> OrtResult<()> {
    ...
    for _ in 0..number{
        let inputs: Vec<Value<'_>> = inputs.iter().map(|input: &ArrayBase<ndarray::CowRepr<'_, f32>, Dim<ndarray::IxDynImpl>>| Value::from_array(session.allocator(),input).unwrap()).collect::<Vec<Value>>();
